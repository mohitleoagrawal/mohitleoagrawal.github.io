...
So this is about making one lane as multi booth by processing multiples vehicles in a go and effectively making number of booths incease to double. One drawback that I could think of this design it might be confusing for users in case where there is no waiting time display placed. They have to predict and go to the fastest lane which can cause a commotion. 
